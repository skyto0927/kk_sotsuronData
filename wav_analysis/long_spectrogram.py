...
N=200
s = 50
hamminngWindow = np.hamming(N)
framerate = 1000
for i in range(1):#人
    for j in range(1):#母音
        for k in range(1):#ピッチ
            for l in range(int(dataset[i][j][k].shape[0] - N)/s)
                data = dataset[i][j][k][i*s:i*s+N]
                spec = 
                freq = np.fft.fftfreq(N, d=1/1000)[:int(N/2)]
