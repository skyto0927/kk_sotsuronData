...
for key in range(1):
    ...
    fname = "recording/KaitoKobayashi/up/a/" + str(int(key)+1) + ".WAV"
    ...
    ran = 100 #1秒で200フレーム
    ave = []
    picklist = [50,250,850,1050,1650,1850]
    ...
    for i in range(6):
        for j in range(50):
            p = picklist[i] + j - 25
            ave[i].append( np.average(cent[p:p+ran]) )
            sig[i].append( np.var(cent[p:p+ran]) )
        ax4.plot(time[picklist[i]:picklist[i]+50], ave[i])
        #ax4.plot(time[picklist[i]:picklist[i]+50], sig[i])
    ...
    ax1.set_title('Up: a to '+vowels[key])
    ...
    fig.savefig("up_a_" +vowels[key]+'.png', bbox_inches="tight", pad_inches=0.05)
